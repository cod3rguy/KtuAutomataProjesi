...
def ressam_backward(ressam,step):
    global oran
    ressam.speed(ressam_hiz())
    ressam.backward(step*(oran/10))
...
def log_yazdir(yapilacaklar: list):
    ...
    while len(command_list) != 0:
        ...
        if command == "F":
            ...
            step = command_list[i+1]
            if dil.get() == 1:
                log_ekrani(satir," "*index + f"{str(step)} step backward.")
            else:
              log_ekrani(satir," "*index + f"{str(step)} Adım geri gidildi.")
            satir += 1.0
            del command_list [i:i+2]
        elif command == "PEN":
            thickness = command_list[i+1]
